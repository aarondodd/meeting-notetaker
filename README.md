...
| Retained format | Opus | What the saved recording becomes after the batch transcription pass finishes. **Opus** (default) trims a 1-hour meeting from ~1 GB to ~45 MB with no practical quality loss for speech. **FLAC** is exactly lossless at ~50% of WAV. **WAV** keeps the source file unchanged (matches v0.6.4 behavior; use if your machine struggles with the post-Stop encode step). Right-click a session in the list to open the recording in your OS media player or delete it. |
