...
| Obsidian note location | `Year / Month` | Template that picks the subfolder + filename. `Year / Month` = `Meetings/{YYYY}/{MM}` (filename = session title); `By series` = `Meetings/{series}` (filename = session title); `By series + date` = `Meetings/{series}/{YYYY}-{MM}-{DD} - {title}` (chronological order inside each series folder); `Flat` = `Meetings/` (filename = session title); `Custom...` exposes a free-form template with `{YYYY}` / `{MM}` / `{DD}` / `{title}` / `{series}` / `{session_id}` placeholders -- if the template's last segment contains `{title}`, it becomes the filename pattern and everything before is the subdir. |
