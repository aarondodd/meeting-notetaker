"""WebRTC-VAD turn segmentation for the loopback audio channel.
...
Uses webrtcvad (already a project dependency) for the per-frame
voiced/silent decision. We previously tried an energy-percentile floor;
that worked on artificial test signals but failed on real meeting audio
where voice dominates (the "floor" calibrates into the voiced range and
the mask collapses to all-silent). webrtcvad has none of that
...
def find_turns(
    ...
    *,
    ...
    if pcm.size == 0:
        return []
    # webrtcvad requires 8/16/32/48 kHz. Resample to 16kHz unconditionally.
    pcm_16k = (
